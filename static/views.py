...
def global_setting(request):
    ...
    #热销榜
    hot_list = Product.objects.all().order_by('-sales')[:4]
    #标签
...
def tags(request):
    try:
        ...
        product_list = Product.objects.filter(tag=tag)
        product_list = getPage(request,product_list)
    except Exception as e:
        logger.error(e)
    return render(request, 'products.html', locals())
...
def detail(request):
    try:
        ...
        try:
            clo = Product.objects.get(pk=did)
...
def getDiscount(request):
    try:
        ...
        product_list = getPage(request,product_list)
        logger.debug("len product_list:%d", len(product_list))
        discount = True
    except Exception as e:
        logger.error(e)
    return render(request, 'products.html', locals())
...
#分页
def getPage(request,product_list):
